...
Dispatcher.subscribeToEvt = function(inUID, inHandler)
	...
	Hax.Log("Subscribed handler to " .. inUID .. " => " .. Hax.Event:UIDToString(inUID))
end
...
Dispatcher.processEvt = function(inEvt)
  --inEvt = tolua.cast(inEvt, "Hax.:Event")
  --tolua.cast(inEvt.UID, "Hax.:EventUID")
  --~ if (inEvt.UID == Hax.EventUID.Connected) then Hax.Log("There's a connected evt!") end

  --~ Hax.Log("Handling an event " .. inEvt.UID)
	done = true
	...
	if (not tracking) then
		...
		if (nrHandlers == 0) then
			Hax.Log("Oops! Found no handlers for event " .. inEvt.UID)
			return true
		end
	...
	Hax.Log("handling evt " .. inEvt.UID .. " => " .. Hax.Event:UIDToString(inEvt.UID))
