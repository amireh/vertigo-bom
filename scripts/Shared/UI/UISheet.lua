local attached = Hax.UI.Attached
...
function UISheet:new(path_to_layout, o)
  ...
  Hax.Log("UISheet: '" .. o.Path .. "' created")
...
function UISheet:attach()
	Hax.Log("UISheet: '" .. self.Path .. "' attached")
...
function UISheet:isAttached()
  return
    ...
    (Hax.UI.CurrentSheet and
    Hax.UI.CurrentSheet.Path == self.Path)
end
...
function UISheet:detach()
  ...
  if self:isAttached() then
    ...
    Hax.UI.CurrentSheet = nil
  end

  layout:setAlwaysOnTop(false)
  layout:hide()

  Hax.Log("UISheet: '" .. self.Path .. "' detached")
...
function UISheet:destroy()
  ...
  local layout = Hax.UI.Dialog.Window:getParent()
  if layout then
    layout:removeChildWindow(Hax.UI.Dialog.Window)
  end
  ...
  Hax.Log("UISheet: '" .. self.Path .. "' destroyed")
  self = nil
...
function UISheet.showDialog(text, callback)
  local dialog = Hax.UI.Dialog
  local self = Hax.UI.CurrentSheet
  local layout = self.Window
...
function UISheet.isDialogShown()
  return Hax.UI.Dialog.Window and Hax.UI.Dialog.Window:isVisible() and Hax.UI.Dialog.Window:getParent()
end
